...
1. **Call Stack**: This is where the code is executed. Functions are pushed onto the stack when they are called and popped off when they return. **Call Stack** operates on the LIFO (Last-In, First-Out) principle. This means that the last function or routine called is the first one to complete and be removed from the stack. When a new function is called, it is pushed onto the top of the call stack. When that function finishes execution, it is popped off the top of the stack, and control returns to the function that called it.
